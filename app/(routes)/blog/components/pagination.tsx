  ...
  */}
      {Array.from({ length: totalPages }, (value, index) => index + 1).map((page) =>
        page === currentPage ? (
          ...
          <Link key={page} href={createPageURL(page)} className="opacity-80 hover:opacity-100 transition-opacity">
            {page}
          </Link>
        )
      )}
      ...
      {currentPage < totalPages ? (
        <Link href={createPageURL(currentPage + 1)} className="ml-2 opacity-90 hover:opacity-100 transition-opacity">
          næste &gt;
        </Link>
      ) : (
